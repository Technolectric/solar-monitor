...
class PersistentLoadManager:
    ...
    def update(self, load_watts):
        ...
        # Keep last 100 readings per hour slot
...
def calculate_battery_breakdown(p_pct, b_volts):
    """Calculates specific kWh segments for the Circular Graph."""
    # 1. Estimate Backup % from Voltage (Linear approx 51V-53V)
    b_pct = max(0, min(100, (b_volts - 51.0) / 2.0 * 100))
    
    # 2. Total Capacities (Wh)
    p_cap = PRIMARY_BATTERY_CAPACITY_WH # 30,000
    b_cap = BACKUP_BATTERY_DEGRADED_WH * BACKUP_DEGRADATION # 14,700
    
    # 3. Current Energy (Wh)
    curr_p = (p_pct / 100) * p_cap
    curr_b = (b_pct / 100) * b_cap
    
    # 4. Calculate Segments based on Cutoffs (40% Primary, 20% Backup)
    # Available = Amount above cutoff
    p_avail = max(0, curr_p - (p_cap * 0.40))
    b_avail = max(0, curr_b - (b_cap * 0.20))
    
    # Reserve = The energy trapped below cutoff (Safety Buffer)
    reserve = (p_cap * 0.40) + (b_cap * 0.20)
    
    # Calculate Total Percentage of 'Usable' (not reserve)
    ...
    return {
        'chart_data': [round(p_avail/1000, 1), round(b_avail/1000, 1), round(reserve/1000, 1)],
        'total_pct': round(total_pct, 1),
        'total_kwh': round(total_current_usable/1000, 1)
    }
...
def poll_growatt():
    ...
    print("🚀 System Started: Tesla Dashboard + Circular Graph")
    ...
    while polling_active:
        try:
            ...
            print(f"Update: Load={tot_out}W | Bat={breakdown['total_pct']}% | Apps={detected}")
...
def home():
    ...
    # Safe Unpacking (FIXES THE 500 ERROR)
    ...
    # Get objects with defaults to prevent NoneType errors
...
<head>
    ...
    <style>
        ...
        /* Tesla Diagram */
        .flow-diagram { position: relative; height: 350px; width: 100%; display: flex; justify-content: center; align-items: center; }
        .node { position: absolute; width: 90px; height: 90px; border-radius: 50%; background: #111; border: 2px solid #333; z-index: 2; display: flex; flex-direction: column; align-items: center; justify-content: center; box-shadow: 0 0 20px rgba(0,0,0,0.5); transition: all 0.3s ease; }
        ...
        .l-solar { top: 100px; } .l-bat { bottom: 100px; }
...
<body>
    ...
    <div class="grid">
        ...
        <div class="col-12 card" style="padding:0">
            <div class="flow-diagram">
                ...
                <div class="node n-solar {{ 'pulse-y' if solar > 50 else '' }}">
                    <div class="node-icon">☀️</div><div class="node-val">{{ '%0.f'|format(solar) }}W</div>
                </div>
                <div class="node n-gen {{ 'pulse-r' if gen_on else '' }}">
                    <div class="node-icon">⚙️</div><div class="node-val">{{ 'ON' if gen_on else 'OFF' }}</div>
                </div>
                <div class="node n-inv"><div class="node-icon">⚡</div><div class="node-val">INV</div></div>
                <div class="node n-home {{ 'pulse-g' if load > 2000 else '' }}">
                    <div class="node-icon">{% if 'Pool' in detected|string %}🏊{% elif 'Water' in detected|string %}🚿{% else %}🏠{% endif %}</div>
                    <div class="node-val">{{ '%0.f'|format(load) }}W</div>
                </div>
                <div class="node n-bat {{ 'pulse-g' if is_charging else ('pulse-r' if is_discharging else '') }}">
                    <div class="node-icon">🔋</div><div class="node-val">{{ breakdown.total_pct }}%</div>
                </div>
            </div>
        </div>
        ...
        <!-- Circular Battery Graph -->
        <div class="col-6 card">
            <h2>Storage Breakdown</h2>
            <div style="height:250px; position:relative">
                <canvas id="doughnutChart"></canvas>
                <div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); text-align:center">
                    <div style="font-family:'Space Mono'; font-size:1.5rem; font-weight:bold">{{ breakdown.total_pct }}%</div>
                    <div style="font-size:0.8rem; opacity:0.7">Usable</div>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 card">
            <h2>Activity</h2>
            {% if detected %}{% for app in detected %}<div class="tag" style="border-color:{{ 'var(--crit)' if 'Water' in app else 'var(--info)' }}">{{ app }}</div>{% endfor %}{% else %}<div style="opacity:0.5">Idle</div>{% endif %}
        </div>
        
        <!-- Alerts -->
        <div class="col-6 card">
            <h2>Alerts</h2>
            {% for a in alerts %}<div class="alert-row"><div style="color:{{ 'var(--crit)' if 'crit' in a.type else 'var(--text)' }}">{{ a.subject }}</div><div class="alert-time">{{ a.timestamp.strftime('%H:%M') }}</div></div>{% endfor %}
        </div>
    ...
    <script>
        ...
        // Circular Battery Chart
        new Chart(document.getElementById('doughnutChart'), {
            type: 'doughnut',
            data: {
                labels: ['Primary (Usable)', 'Backup (Usable)', 'Reserve (Cutoff)'],
                datasets: [{
                    data: {{ breakdown.chart_data|tojson }},
                    backgroundColor: ['#3fb950', '#ffa500', '#333333'],
                    borderWidth: 0
                }]
            },
            options: { ...common, cutout: '70%', plugins: { legend: { position: 'right' } } }
        });
        ...
        new Chart(document.getElementById('simChart'), {
            ...
            options: { ...common, scales: { x:{grid:{display:false}}, y:{min:0, max:100, grid:{color:'rgba(255,255,255,0.1)'}} } }
        });
